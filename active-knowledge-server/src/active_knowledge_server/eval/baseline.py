...
def compare_against_baseline(
	*,
	...
	warnings = tuple(metrics["warnings"])
...
def build_regression_gate_metrics(
	*,
			...
			failures.append(
				{
					...
					"metric": metric,
					"message": "quality regression comparison requires both current and baseline values",
				}
			)
			continue
	...
	if mrr_current is None or mrr_baseline is None:
		...
		if mrr_current < QUALITY_GATE_THRESHOLDS["mrr"] or (mrr_baseline - mrr_current) > _MRR_MAX_DROP:
			failures.append(
				{
	...
	performance_comparison = build_performance_regression_summary(
		current_performance_gate=_mapping(summary_get(current_performance_report.metrics, "performance_gate")),
		baseline_performance_gate=_mapping(summary_get(baseline.performance_report.metrics, "performance_gate")),
	)
	...
	*,
	...
	for probe_id, (unit, _threshold) in PERFORMANCE_GATE_THRESHOLDS.items():
		...
		if regression_ratio > _PERFORMANCE_WARNING_RATIO:
			warnings.append(
				{
					"check": "performance_regression_warning",
					"probe_id": probe_id,
					"unit": unit,
					"baseline_p95": baseline_p95,
					"current_p95": current_p95,
					"regression_ratio": round(regression_ratio, 6),
					"warning_ratio": _PERFORMANCE_WARNING_RATIO,
				}
			)
