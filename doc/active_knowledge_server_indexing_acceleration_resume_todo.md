...
- [ ] 新增 `IndexPlanSignature` 或 `make_index_plan_signature(...)` helper。
- [ ] signature 输入包含：mode、target、source、snapshot_id、workspace inventory hash、source docs manifest hash、parser schema、profile relation schema、embedding provider/model/enabled、影响解析的 config hash、storage schema version。
- [ ] 明确 `workers`、writer batch size、commit interval 不进入 signature。
- [ ] 对 signature payload 使用 sorted JSON + sha256，保证跨进程稳定。
- [ ] 单测覆盖同输入稳定、无关配置变化不变、parser/embedding/manifest 变化必变。
...
- [ ] 定义 task key 格式：`code:apply:<path>`、`code:delete:<path>`、`doc:apply:<path>`、`doc:delete:<path>`、`vector:doc:<path>`、`profile:relations`、`workspace:map`。
- [ ] 从 `IncrementalIndexPlan` 派生稳定排序 task list。
- [ ] 每个 task 携带 `phase`、`source_kind`、`relative_path`、`input_hash`、`schema_version`、`embedding_model`、`required`。
- [ ] 将现有 `_incremental_code_paths_to_collect`、`_incremental_doc_paths_to_collect` 的结果纳入 collect task 依赖。
- [ ] 单测覆盖全量重建、少量 code/doc 变更、删除、vector rebuild、profile 变更。
