...
- [x] 新增 worker 解析函数：`auto` 根据 CPU、文件数和阶段类型给出保守值。
- [x] `workers=1` 强制串行，作为快速回退开关。
- [x] 记录实际 worker 数到 result metadata，方便压测和问题追踪。
- [x] 对空任务、单任务、小仓自动使用串行，避免线程池开销大于收益。
...
- [x] 基于 `ThreadPoolExecutor` 实现 `parallel_map_ordered` 或等价 helper。
- [x] 支持 `max_in_flight`，避免大仓一次性提交全部任务导致内存峰值过高。
- [x] 支持 per-item 异常包装，返回 path + error + warning，不让单文件失败杀死整个阶段。
- [x] 支持 callback 上报 `current_path` 和 done count。
- [x] 输出按输入 path 排序，保证后续写入和测试快照稳定。
...
- 完成产物：
  [doc_indexer.py](/home/gangan/GANLab/ActiveTools/active-knowledge/active-knowledge-server/src/active_knowledge_server/indexing/doc_indexer.py:178)
  [test_doc_indexer.py](/home/gangan/GANLab/ActiveTools/active-knowledge/active-knowledge-server/tests/unit/test_doc_indexer.py:246)

TODO：

- [x] 将 `DocumentIndexer.collect` 中单文档解析和记录构建拆成私有方法，如 `_collect_document_entry(...)`。
- [x] source records 仍按 manifest/category 串行构建。
- [x] 每个 worker 只返回当前文档的 file/chunk/entity/evidence/embedding input/warnings。
- [x] embedding preparation 可先在主线程统一处理，避免 secret scanner 与向量写入跨线程复杂化。
- [x] 归并后按 `storage_relative_path`、`ordinal`、稳定 ID 排序。
...
- 完成产物：
  [code_indexer.py](/home/gangan/GANLab/ActiveTools/active-knowledge/active-knowledge-server/src/active_knowledge_server/indexing/code_indexer.py:151)
  [test_code_indexer.py](/home/gangan/GANLab/ActiveTools/active-knowledge/active-knowledge-server/tests/unit/test_code_indexer.py:229)

TODO：

- [x] 将读取、hash、`FileRecord`、C family parse、Makefile parse 拆成 `_collect_code_entry(...)`。
- [x] 目录 anchor、directory contains relation、跨文件汇总关系仍在主线程确定性构建。
- [x] Makefile sibling scan 保持可控，失败只产生当前文件 warning。
- [x] 归并 `code_parses`、`makefile_parses`、`file_texts` 时按 relative path 排序。
- [x] 对 ctags/tree-sitter 等非线程安全依赖做确认；v1 线程池只调用当前无共享可变状态的 parser 函数，后续若启用新 parser 再加串行 fallback。
...
- 完成产物：
  [pipeline.py](/home/gangan/GANLab/ActiveTools/active-knowledge/active-knowledge-server/src/active_knowledge_server/indexing/pipeline.py:485)
  [code_indexer.py](/home/gangan/GANLab/ActiveTools/active-knowledge/active-knowledge-server/src/active_knowledge_server/indexing/code_indexer.py:179)
  [test_incremental_pipeline.py](/home/gangan/GANLab/ActiveTools/active-knowledge/active-knowledge-server/tests/unit/test_incremental_pipeline.py:588)

TODO：

- [x] 为 `CodeIndexer.collect` 增加 `include_paths` 或 bundle-level collect API。
- [x] 当 `plan.reindex_all_code=False` 时只 collect `changed_code_paths` 需要的 bundle。
- [x] 保留 reindex all 场景的整仓 collect 路径。
- [x] 文档路径继续使用 filtered manifest，但改为批量 filtered manifest + 并行 collect，而不是逐文档调用 `collect`。
...
- [x] code/doc collect 阶段并行产生内存 bundle。
- [x] apply 阶段仍在主线程按 `relative_path` 排序调用 `_apply_code_bundle`、`_apply_doc_bundle`。
- [x] vector upsert 仍在主线程，先不做多线程写 vector store。
- [x] progress 事件区分 collect 和 apply，便于定位是解析慢还是写入慢。
