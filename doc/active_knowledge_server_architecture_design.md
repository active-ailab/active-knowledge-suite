...
- P95 延迟相对上一发布基线退化超过 20% 时默认为 `blocker`；只有带明确原因的发布豁免可降级为 `warning` 并写入回归报告；导致超时或工具不可用时始终为 `blocker`。
...
| 需求 | Gate 检查 | 阻断级别 |
| --- | --- | --- |
| 可重复索引 | `active-kb eval run --gate reproducibility` 连续收集同一 snapshot/profile，entity/chunk/evidence/vector stable ID 和核心 report hash 一致。 | `blocker` |
